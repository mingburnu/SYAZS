	...
	<c:when test="${empty viewSerNo}">
		<script type="text/javascript">
			...
			function closeDetail_ToQuery() {
				...
				UI_Resize();
				if ($(
						"form#apply_referenceOwner_list input#listForm_currentPageHeader")
						.val() != null) {
					gotoPage($(
							"form#apply_referenceOwner_list input#listForm_currentPageHeader")
							.val());
...
<c:if test="${empty entity.serNo}">
	<script type="text/javascript">
		function reimport() {
			goDetail("<c:url value = '/'/>crud/apply.referenceOwner.paginate.action?pager.currentPage=${pager.currentPage}&pager.recordPerPage=${pager.recordPerPage}");
		}
	</script>
</c:if>
</head>
