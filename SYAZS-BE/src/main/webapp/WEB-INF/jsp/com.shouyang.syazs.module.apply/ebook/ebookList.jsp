...
<script type="text/javascript">
	...
	function goSearch() {
		goMain("<c:url value = '/'/>crud/apply.ebook.list.action",
				"#apply_ebook_list", "");
	}

	//新增
	function goAdd() {
		goDetail("<c:url value = '/'/>crud/apply.ebook.add.action", '電子書-新增');
	}
		...
		if (IsSelected) {
			var f = {
				...
				trueFunc : function() {
					...
					var data = $('#apply_ebook_list').serialize()
	...
	function goUpdate(serNo) {
		...
		if (isNum && parseInt(serNo) > 0) {
			goDetail("<c:url value = '/'/>crud/apply.ebook.edit.action?"
					+ 'entity.serNo=' + serNo, '電子書-修改');
		}
	}

	//批次匯入
	function goImport() {
		goDetail("<c:url value = '/'/>crud/apply.ebook.imports.action",
				'電子書-匯入');
	}
</script>
</head>
