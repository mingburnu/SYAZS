...
<script type="text/javascript">
	...
	function addAllBox() {
		...
		$.ajax({
			type : "POST",
			url : "<c:url value = '/'/>crud/apply.ebook.addAllItem.action",
			dataType : "html",
	...
	function removeAllBox() {
		...
		$.ajax({
			type : "POST",
			url : "<c:url value = '/'/>crud/apply.ebook.removeAllItem.action",
			dataType : "html",
			success : function(message) {
	...
	function allRow(action) {
		...
		if (action == 1) {
			...
			$
					.ajax({
						type : "POST",
						url : "<c:url value = '/'/>crud/apply.ebook.allUncheckedItem.action",
						dataType : "html",
						data : importItem.slice(0, importItem.length - 1),
						success : function(message) {
	...
	function getErrors() {
		var url = "<c:url value = '/'/>crud/apply.ebook.backErrors.action?entity.option=errors";
		window.open(url, "_top");
	}

	function getTips() {
		var url = "<c:url value = '/'/>crud/apply.ebook.backErrors.action?entity.option=tips";
		window.open(url, "_top");
	}
