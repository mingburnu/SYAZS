...
<script type="text/javascript">
	...
	//匯出範本
	function goExample() {
		var url = "<c:url value = '/'/>crud/apply.ipRange.example.action";
		window.open(url, "_top");
	}

	//Excel列表
	function goQueue_detail() {
		...
		var formObj = $("form#apply_ipRange_queue");
		var formURL = $("form#apply_ipRange_queue").attr("action")
		...
		//			if(false)
		{
			...
			var formData = new FormData(document
					.getElementById("apply_ipRange_queue"));
			$.ajax({
				url : formURL,
				type : 'POST',
				data : formData,
		...
		} else //for olden browsers
		{
			...
			iframe.load(function(e) {
				...
				UI_Resize();
				$(window).scrollTop(0);
				$("#div_Detail_2 .content > .header > .title").html("IP-匯入");
				$("#div_Detail_2 .content > .contain").empty().html(data);
				closeLoading();
			});
		}
...
<body>
	<s:form namespace="/crud" action="apply.ipRange.queue"
		...
		<div class="button_box">
			<div class="detail-func-button">
				<a class="state-default" onclick="closeDetail_2();">取消</a> &nbsp;<a
					...
					id="ports" class="state-default" onclick="goQueue_detail();">下一步</a>
			</div>
		</div>
