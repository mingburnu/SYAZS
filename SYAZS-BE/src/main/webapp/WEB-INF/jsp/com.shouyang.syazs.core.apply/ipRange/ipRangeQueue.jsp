...
<script type="text/javascript">
	...
	function addAllBox() {
		...
		$.ajax({
			type : "POST",
			url : "<c:url value = '/'/>crud/apply.ipRange.addAllItem.action",
			dataType : "html",
	...
	function removeAllBox() {
		...
		$
				.ajax({
					type : "POST",
					url : "<c:url value = '/'/>crud/apply.ipRange.removeAllItem.action",
					dataType : "html",
					success : function(message) {
	...
	function allRow(action) {
		...
		if (action == 1) {
			...
			$
					.ajax({
						type : "POST",
						url : "<c:url value = '/'/>crud/apply.ipRange.allCheckedItem.action",
						dataType : "html",
			...
			$
					.ajax({
						type : "POST",
						url : "<c:url value = '/'/>crud/apply.ipRange.allUncheckedItem.action",
						dataType : "html",
						data : importItem.slice(0, importItem.length - 1),
						success : function(message) {
	...
	function getCheckedItem(index) {
		$
				.ajax({
					type : "POST",
					url : "<c:url value = '/'/>crud/apply.ipRange.getCheckedItem.action",
					dataType : "html",
	...
	function getErrors() {
		var url = "<c:url value = '/'/>crud/apply.ipRange.backErrors.action";
	...
	function checkData() {
		...
		if ($("input.checkbox.queue:checked").length > 0) {
			goDetail_Sub(
					'<c:url value = '/'/>crud/apply.ipRange.importData.action',
					'#apply_ipRange_paginate', '&pager.currentPage='
...
<body>
	<s:form namespace="/crud" action="apply.ipRange.paginate" method="post"
			...
			<tbody>
				...
				<c:forEach var="item" items="${ds.results}" varStatus="status">
					<tr>
						...
						<td><esapi:encodeForHTML>${item.ipRangeStart }</esapi:encodeForHTML></td>
						<td><esapi:encodeForHTML>${item.ipRangeEnd }</esapi:encodeForHTML></td>
						<td><esapi:encodeForHTML>${item.dataStatus }</esapi:encodeForHTML></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
					...
					<c:if test="${ds.pager.totalRecord > 0 }">
						<tr>
							<td><jsp:include page="/WEB-INF/jsp/layout/pagination.jsp">
									...
									<jsp:param name="action" value="apply.ipRange.paginate" />
									<jsp:param name="pager" value="${ds.pager}" />
									<jsp:param name="detail" value="2" />
								</jsp:include></td>
							<td>每頁顯示 <select id="listForm_pageSize"
								name="pager.recordPerPage" onchange="changePageSize_detail_2()">
								...
								max="${totalPage }" onchange="gotoPage_detail_2(this.value)">
		...
		<div class="button_box">
			<div class="detail-func-button">
				<a class="state-default" onclick="allRow(1)">全選</a> <a
					...
					class="state-default" onclick="closeDetail_2();">關閉</a> <a
