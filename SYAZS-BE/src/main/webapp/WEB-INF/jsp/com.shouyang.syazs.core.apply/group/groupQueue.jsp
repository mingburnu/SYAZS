...
<script type="text/javascript">
	...
	function addAllBox() {
		...
		$.ajax({
			type : "POST",
			url : "<c:url value = '/'/>crud/apply.group.addAllItem.action",
			dataType : "html",
	...
	function removeAllBox() {
		...
		$.ajax({
			type : "POST",
			url : "<c:url value = '/'/>crud/apply.group.removeAllItem.action",
			dataType : "html",
			success : function(message) {
	...
	function allRow(action) {
		...
		if (action == 1) {
			...
			$
					.ajax({
						type : "POST",
						url : "<c:url value = '/'/>crud/apply.group.allCheckedItem.action",
						dataType : "html",
			...
			$
					.ajax({
						type : "POST",
						url : "<c:url value = '/'/>crud/apply.group.allUncheckedItem.action",
						dataType : "html",
						data : importItem.slice(0, importItem.length - 1),
						success : function(message) {
	...
	function checkData() {
		...
		if ($("input.checkbox.queue:checked").length > 0) {
			goDetail_Sub(
					'<c:url value = '/'/>crud/apply.group.importData.action',
					'#apply_group_paginate', '&pager.currentPage='
...
<body>
	<s:form namespace="/crud" action="apply.group.paginate" method="post"
			...
			<tbody>
				...
				<c:forEach var="item" items="${ds.results}" varStatus="status">
					<tr>
						...
						<td><esapi:encodeForHTML>${item.firstLevelGroup.groupName }</esapi:encodeForHTML></td>
						<td><esapi:encodeForHTML>${item.secondLevelGroup.groupName }</esapi:encodeForHTML></td>
						<td><esapi:encodeForHTML>${item.thirdLevelGroup.groupName }</esapi:encodeForHTML></td>
